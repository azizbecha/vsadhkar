...
const makeDuaKey = (dua: Dua): string =>
  `${dua.arabic}||${dua.transliteration}||${dua.translation}`;

const isSameDua = (a: Dua, b: Dua): boolean => makeDuaKey(a) === makeDuaKey(b);

const findDuaByKey = (key: string): Dua | undefined =>
  allDuas.find((dua) => makeDuaKey(dua) === key);
...
const showDua = (context: vscode.ExtensionContext) => {
  ...
  vscode.window.showInformationMessage(
    duaLanguage === "translation" ? dua.translation : dua.arabic,
  );
};
...
const fetchCountries = async (): Promise<Country[]> => {
  const key = "countries";
  if (geoCache.has(key)) {
    return geoCache.get(key) as Country[];
  }
...
const fetchStates = async (countryIso: string): Promise<State[]> => {
  ...
  if (geoCache.has(key)) {
    return geoCache.get(key) as State[];
  }

  const response = await fetch(
    `${SERVER_URL}/api/countries/${countryIso}/states`,
  );
...
const fetchCities = async (
  ...
  if (geoCache.has(key)) {
    return geoCache.get(key) as City[];
  }

  const response = await fetch(
    `${SERVER_URL}/api/countries/${countryIso}/states/${stateIso}/cities`,
  );
...
const fetchPrayerTimes = (
  ...
  const method = context.globalState.get<number>(
    "vsadhkar.calculationMethod",
    3,
  );
  fetch(
    `http://api.aladhan.com/v1/timingsByCity?country=${country}&state=${state}&city=${city}&method=${method}`,
  )
...
const selectLocation = async (context: vscode.ExtensionContext) => {
  try {
    ...
    const selectedCountry = await vscode.window.showQuickPick(countryItems, {
    ...
    const selectedState = await vscode.window.showQuickPick(stateItems, {
      placeHolder: `Select a state in ${selectedCountry.label}`,
    });
    if (!selectedState) {
      return;
    }
    ...
    if (selectedState.type !== null) {
      ...
      if (cities.length > 0) {
        ...
        const selectedCity = await vscode.window.showQuickPick(cityItems, {
          placeHolder: `Select a city in ${selectedState.label}, ${selectedCountry.label}`,
        });
        if (!selectedCity) {
          return;
        }
    ...
    vscode.window.showInformationMessage(
      `Selected Location: ${cityLabel}, ${selectedState.label}, ${selectedCountry.label}`,
    );
...
function schedulePrayerNotifications(prayerTimesObj: {
  [key: string]: string;
}) {
  ...
  Object.entries(prayerTimesObj).forEach(([prayer, timeStr]) => {
    ...
    const notifyAt = new Date(
      prayerTime.getTime() - notifyBeforeMinutes * 60 * 1000,
    );
    const delay = notifyAt.getTime() - now.getTime();
    if (delay > 0) {
      prayerNotificationTimeouts.push(
        setTimeout(() => {
          vscode.window.showInformationMessage(
...
function setupAfterLocation(context: vscode.ExtensionContext) {
  ...
  if (!nextPrayerItem) {
    nextPrayerItem = vscode.window.createStatusBarItem(
...
export function activate(context: vscode.ExtensionContext) {
  ...
  getDhikrStatusBarButton = vscode.window.createStatusBarItem(
    vscode.StatusBarAlignment.Right,
    100,
  );
  ...
  activeScreen = context.globalState.get<"main" | "favorites" | "settings">(
    "vsadhkar.activeScreen",
    "main",
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("vsadhkar.clearData", async () => {
      ...
      vscode.window.showInformationMessage(
        "VSAdhkar: extension data cleared. Reload window to test first-run flow.",
      );
    }),
  );
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(
      ExampleSidebarProvider.viewType,
      provider,
    ),
  );
...
class ExampleSidebarProvider implements vscode.WebviewViewProvider {
    ...
    webviewView.webview.onDidReceiveMessage((message) => {
      switch (message.command) {
        case "saveSettings": {
          ...
          vscode.window.showInformationMessage(
            `Settings saved: Show Dua every ${verboseMs(t)}`,
          );
        ...
        case "toggleFavoriteDua": {
          ...
          const exists = favoriteDuas.some((dua) =>
            isSameDua(dua, currentDua!),
          );
          if (exists) {
            favoriteDuas = favoriteDuas.filter(
              (dua) => !isSameDua(dua, currentDua!),
            );
          } else {
            favoriteDuas = [...favoriteDuas, currentDua];
          }
          this._context.globalState.update(
            "vsadhkar.favoriteDuas",
            favoriteDuas,
          );
          provider.updateWebview();
          break;
        ...
        case "pickFavoriteDua": {
          ...
          const selected =
            findDuaByKey(key) ??
            favoriteDuas.find((dua) => makeDuaKey(dua) === key);
          if (!selected) {
            break;
          }
          currentDua = selected;
        ...
        case "tasbihComplete": {
          vscode.window.showInformationMessage(
            `🎉 Tasbih complete! ${tasbihTarget}/${tasbihTarget}`,
          );
          break;
        }
      }
  ...
  public updateWebview() {
    if (this._view) {
      updateLocationAndPrayerTimes(this._context);
      timeInterval = this._context.globalState.get(
        "vsadhkar.timeInterval",
        30000,
      );
  ...
  private getHtmlForWebview(webview: vscode.Webview): string {
    ...
    const palestineFlag = webview.asWebviewUri(
      vscode.Uri.joinPath(this._extensionUri, "media", "palestine.png"),
    );
    const prayerTimes: { [key: string]: string } | undefined =
      this._context.globalState.get("prayerTimes");
    const sunTimings: { [key: string]: string } | undefined =
      this._context.globalState.get("sunTimings");
    const hasLocation =
      country !== undefined &&
      state !== undefined &&
      city !== undefined &&
      prayerTimes !== undefined;
    const isCurrentFavorite = currentDua
      ? favoriteDuas.some((dua) => isSameDua(dua, currentDua!))
      : false;
      ...
      ? `
            ...
            <div class="section">
                ...
                <div class="prayer-list">
                    ${(["Imsak", "Sunrise", "Sunset"] as const)
                      .filter((k) => sunTimings[k])
                      .map(
                        (k) => `
